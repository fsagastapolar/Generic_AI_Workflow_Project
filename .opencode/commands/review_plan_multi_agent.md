...
First, extract `AUTHOR` from the plan document start information by looking for:

`AI Author: <value>`

If that field exists and is unambiguous, use it directly.

If the field is missing/ambiguous, ask the user via `question` (or your interactive prompt mechanism):
...
Capture `review_start_time` (local timestamp) right before dispatching reviewers. Capture `review_end_time` after the consolidated review is saved. Compute duration in hours/minutes for the plan header.
...
## Step 8: Update the Plan Header + Annex

After the consolidated review is saved, update the **plan file itself** with review metadata and an annex entry.
...
## Review History
- Review [N]: [YYYY-MM-DD HH:MM] — Duration: [Hh Mm], Reviewers: [Claude, Gemini, Codex], Verdict: [APPROVE/REVISE/REJECT], Review: `[consolidated_review_path]`
```
...
- **Brief**: [1-3 bullets summarizing the most important consolidated findings and recommended fixes]
```

Use reviewer attribution from the consolidated review for `Found By`.
...
1. Apply the review now — continue in this session and run the apply-review workflow as a subtask
...
## Step 11: Act on the Decision

- **Apply the review**: Do not ask the user to run another command manually. Continue in this session. Preferred: invoke `/apply_review [plan_path]` as a subtask. Fallback: run the equivalent apply-review workflow directly by dispatching `plan-reviser` with the consolidated review path and then present the revision summary.
...
7. **No command ping-pong.** When the user chooses a next action that maps to another workflow (especially apply review), execute it in-session via subtask/delegation instead of asking the user to run a new command manually.
8. **The agents are mean, you are diplomatic.** Present the consolidated review honestly but help the user act on it constructively.
