...
- Navigate to actual URLs (e.g., `http://localhost:3000/login`)
...
## Pre-Test Browser State Reset (MANDATORY)

**Before executing ANY test or scenario**, you MUST wipe all browser state from the Playwright browser context. This ensures a clean, deterministic baseline every time.

Execute the following steps in order:

### Step 1: Navigate to a Blank Page

```
browser_navigate(url="about:blank")
```

This ensures you are not on a page that could block storage clearing.
...
await Promise.all([
  ...
  // Expire all cookies for the current origin
...
### Step 3: Force HTTP Cache Bypass on First Navigation

> **Why this matters**: Vite serves Vue files as ES modules (e.g. `http://localhost:3000/src/views/MyView.vue`). Even with `--isolated`, the browser's in-memory HTTP cache can serve a stale module within the same MCP session. A cache-busting first request forces Vite to re-serve all modules fresh.

Navigate to the app root with a timestamp query param first, then to your actual target:

```
browser_navigate(url="http://localhost:<PORT>/?_nocache=<TIMESTAMP>")
# wait 500ms, then navigate to the real URL:
browser_navigate(url="http://localhost:<PORT>/actual/path")
```

Replace `<PORT>` and `<TIMESTAMP>` with the actual values from the test spec (e.g. `3000` and `Date.now()`).

> **Note**: The `--isolated` flag in the Playwright MCP config (`mcp_playwright.json`) ensures the disk-based HTTP cache is never written between sessions. This Step 3 handles any residual in-memory cache within the current session.

### Step 4: Confirm Clean State

After clearing, verify the reset succeeded:
```javascript
// Should return empty or null for all
const state = {
...
console.log("Browser state after reset:", JSON.stringify(state));
```

**This reset is NON-NEGOTIABLE.** Do not skip it even if you believe state is already clean. Stale cookies, cached tokens, or leftover Pinia hydration can cause false positives or intermittent failures.
...
1. **Verify Frontend is Running**:
   - Navigate to the configured frontend URL
...
1. ✅ **Reset browser state before every test run** - clear localStorage, sessionStorage, IndexedDB, cookies, and Playwright cache
2. ✅ **Always wait for reactivity** after Vue state changes (300ms default)
...
❌ **Assuming state**: ALWAYS run the Pre-Test Browser State Reset before every test — never assume a clean state
